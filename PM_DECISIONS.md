...
#### D.23 Generate Reflection button — reframe of B.17-ext refresh affordance
Stream 0.5p piece #3. The manual ↻ refresh affordance (B.17-ext, Stream 0.5j-fix2) read as "something broke, refresh it" per real-user testing — repair framing rather than reward. Reframed conditionally: when all unlabeled transactions have been labeled (`!hasUnlabeled`), a sage-tinted "Generate reflection" pill button appears beside the patterns header. When unlabeled items remain, the affordance is hidden — labeling is the user's actual job, not refreshing. Button stays enabled after tap (option c per spec §6.5) so users can regenerate. `RotateCcw` icon retired from this file. Underlying `forceResynthesizePatterns()` logic from 0.5j-fix2 era unchanged.
...
#### D.25 Stream 0.5p piece #7 (Reflect trend visualization) deferred to V2 — pre-flight blocker
Spec §9.2 STOP clause triggered during pre-flight: all 14 reflections in DB carry `reflected_at: 2026-05-28` (same-day, set by the seed/auto-reset stamp cycle). No `reflected_at`-based time-bucket trend is computable. Three paths the spec contemplated: (a) seed reshape, (b) defer entirely, (c) bucket by `transactions.occurred_at` instead — which would shift the semantic from "your labeling behavior over time" to "your purchase patterns over time." Banked as V2 work pending PM confirmation of (c) — the simpler honest path for pre-Monday is defer. Documents the STOP-clause discipline working as intended: a stream's pre-flight catches a scope-blocker before code touches it. Same family as D.19 (per-merchant capability already shipped) and D.5 (reset_april_ritual false-alarm).
