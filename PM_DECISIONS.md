...
Last updated: 2026-05-29 — Phase 3 complete (tag `phase-3-complete`). All Phase A/B/C/D work shipped: B1 Profile expansion · B2 v2 Reflect hybrid · B3 Goals · C1 Monthly Ritual 7-screen · C2 WindfallFlow · C3 Verdict Cards · C4 Onboarding · 0.5j-n polish streams · D audit + lint sweep. Stream 0.5p (8 pre-delivery enhancements) shipped under Phase 3 Build B.18 + C.26 + C.27 + C.28 + D.20–D.24 (D.25 resolved via Path B per B.18). Stream 0.5q (6 post-0.5p refinements) shipped D.27 / D.29 / D.30 / D.31 / D.32 (D.23 + D.28 superseded by D.32). Stream 0.5r (6 post-0.5q refinements) shipped D.33 / D.34 / D.35 / D.36 / D.37 / D.38. Stream 0.5s (7 pieces) shipped D.40 AI prose replaced with deterministic rule engine (synthesize-patterns Edge Function dormant) · D.41 sticky button hides post-generation · D.42 auto-scroll to chart on generation complete · D.43 Reflect post-generation surface redesigned to chart-primary with per-merchant trend cards + rule-engine prose behind "Know more" expand · D.44 (addendum) swap one Zerodha SIP off Reflect labeling list for a Bblunt Salon ₹2,800 April-9 row · D.45 (addendum) deriveEmotionHeadline switched from count-based to rate-based comparison + branch order reshuffled so strong-improving wins over crossover (Priya canonical headline now reads "You're trending toward worth-it" per Gate H.A). Stream 0.5t (9 pieces) shipped D.47 Priya income raise ₹68,500 → ₹98,000 · D.48 "Buffer floor" → "Safety net" rename across code + UI + PM_DECISIONS · D.49 user-rules schema (`safety_net` / `impulse_wait_threshold` / `impulse_wait_hours` / `daily_sps_floor` columns + source-of-truth modules) · D.50 onboarding "Your rules" step capturing user-input thresholds · D.51 chat grounding drift fix + citation mandate extended to verdict_line + body · D.52 `rule_citations[]` array on structured verdict response + VerdictCard badge rendering · D.53 chat audit re-baseline to new income + new Cases 8/9 for impulse-wait boundary · D.54 SafeToSpendHero relabel ("Safe to spend daily this month" + "Salary next on {date}", with payday bump fix). Stream 0.5u (5 pieces) shipped D.55 windfall card single-Skip dismiss with session-state fix · D.56 Windfall destination page → "Allocate" · D.57 "Commitments" → "Fixed commitments" sweep (Home, Profile, chat grounding) · D.58 PatternsCallout removed from Home · D.59 "For you today" → "Track your goals". Stream 0.5v (5 pieces) shipped D.60 close-out math-reveal recap card + "Spending leftover" rename + leftover demoted to recap row (delivers the never-shipped D.26 from 0.5p) · D.61 "Looking back" reflection-labeling section removed from close-out (Reflect tab owns reflection) · D.62 "What you can do now" rule-engine guidance with four severity tiers (small_short / deficit_safe / deficit_breached / repeated_deficit) + March 2026 negative seed + Reviewer Console preview link. C.11 amended to reflect post-D.49 schema-backed state. Banked for post-delivery: divergence test artifact (Phase 3 Build C.23) and case study writeup (Phase 3 Build C.24).
...
#### C.11-feat Safety net + impulse wait + daily SPS floor as derived constants — schema-backed since D.49
Phase C3 grounding context injected ₹1,00,000 buffer floor (renamed to "safety net" in Stream 0.5t D.48), 48-hour wait above ₹2,000 (corrected to ₹3,000 in Stream 0.5t D.51, aligning prompt with the Profile UI value that had drifted), ₹300 daily SPS floor. Originally hardcoded in `buildGroundingContext`; moved to `profiles` schema columns in Stream 0.5t D.49 with a `getUserRulesFromProfile()` helper. V2 work flagged here ("expose as editable profile settings") shipped under Stream 0.5t D.50 (onboarding capture) — Profile edit affordance still V2.
...
#### D.44 Reflect labeling list — swap one leaky SIP for a reflectable salon visit
Stream 0.5s addendum (2026-05-29). Real-user testing of the post-D.43 Reflect tab flagged that the labeling-candidate list (`fetchUnlabeledRecent`, top 8 by date-DESC after the `>₹1,000` floor) was dominated by leaky fixed-commitment transactions users dismiss with the X-icon rather than reflecting on: Electricity, Health Insurance, two Zerodha Coin SIP rows. Net effect: only 2 of the 8 visible items (Myntra ₹4,800 + Amazon ₹1,950) were actually reflectable; the rest were process noise.

Fix without breaking commitment math: add one new April-9 discretionary transaction (`Bblunt Salon ₹2,800`, Personal Care, `commitment_id` NULL, `is_significant=true`) into the seed. The new row ranks between Apr 11 Toit (slot 6) and Apr 7 SIPs (slots 7-8) in the Reflect query's DESC order, so it bumps the smaller `SIP MF 2 ₹5,000` (Zerodha Coin) off the top-8 visible list. The SIP transaction stays in DB — `CommitmentsCard`, ritual close-out, commitment-month math all see it; only the Reflect labeling surface skips it via natural date ordering.
...
#### D.47 Priya income raise — ₹68,500 → ₹98,000
Stream 0.5t piece #1. Real-user testing on the post-0.5s state flagged that Priya's income-to-commitments ratio was implausible: ₹68,500 net against ₹47,468 non-investing commitments + ₹15,000 investing SIPs + ₹9,000 active goal contributions left effectively ~₹12K discretionary, but with goal contributions deducted from the displayed safe-to-spend (per the canonical formula) the surface felt strained for a Strategist-archetype supporting-dependents persona. Raised `monthly_income_net` to ₹98,000 (gross ₹1,25,000). New month SPS: ₹98,000 − ₹47,468 − ₹9,000 = ₹41,532; new daily SPS: ₹41,532 / 31 ≈ ₹1,340. Healthier proportion that lines up with how a tier-1 Indian working professional of Priya's profile would actually live.

Cascade applied through every income-dependent surface: seed transactions (salary credits for all 6 months bumped to ₹98K), `monthly_rituals` lock-ins (Jan/Feb/Mar 2026 `income_confirmed` + `safe_to_spend_locked` re-baselined to ₹98,000 / ₹41,500-41,700 with the same ±₹500 month-to-month variance the original seed had), `CLAUDE.md` persona row, `ProfilePage.tsx` default fallback, `OnboardingPage.tsx` (manual-entry placeholder + statement-detected display + Ready-step synthesis fallback), `phasec1-lockin-check.mjs` RPC call values, `tests/unit/safeToSpend.test.ts` fixture, `test-chat-7cases.mjs` Case 1 + Case 2 expectations. Historical ₹12,032 references in `dailySafeToSpend.ts` + `hallucination_guard.ts` are doc-comments only (no runtime logic), left as-is. Persona-internal change — no schema, no architecture change.

#### D.48 "Buffer floor" → "Safety net" rename across all surfaces
Stream 0.5t pieces #2-3. Real-user testing on 0.5s state flagged "buffer floor" as finance-speak that asks the user to translate to intent ("oh, that's the emergency money"). Per the C.16 / C.26 / D.27 / D.34 voice-discipline family (resist LLM-default / finance-jargon language in favor of colloquial user-intention phrasing), renamed everywhere:

- **User-facing strings.** ProfilePage "Your rules" label changed from "Buffer floor" to "Safety net". `prompt_builder.ts` grounding text changed from "Buffer floor: maintain..." to "Safety net (slug: ...): maintain...". Chat verdict prompt language updated where it referenced "buffer rule" / "buffer floor".
- **Schema column.** Migration 0019 created `profiles.safety_net` (not `buffer_floor`) — single concept end-to-end. Resolves the documented C.11 V2 deferred work in the same stroke.
- **PM_DECISIONS prose.** C.11 amendment updated to reflect the rename and the post-D.49 schema-backed state. Historical "buffer floor" references are documented in C.11's body, so anyone reading the decision log can follow the naming evolution.

"Safety net" reads as how a user would describe the rule to a friend (*"I keep a safety net of ₹1L"*) — colloquial, matches the rule's purpose intuitively, doesn't require translation. Pairs with the voice-discipline thread that runs through the entire build: resist the LLM's default phrasing, choose the words a thoughtful user would actually use.

#### D.49 User rules moved from hardcoded constants to schema-backed columns
Stream 0.5t pieces #4-5. Resolves the C.11-banked V2 deferred work ("expose as editable profile settings"). Migration 0019 adds four `INTEGER NOT NULL DEFAULT` columns to `profiles`: `safety_net` (default 100000), `impulse_wait_threshold` (default 3000 — corrects the pre-0.5t drift between prompt_builder's ₹2,000 and the Profile UI's ₹3,000; the Profile UI value wins because it's what the user was reading), `impulse_wait_hours` (default 48), `daily_sps_floor` (default 300). 0019 also backfills Priya with explicit values matching the defaults so a partial-apply (just 0019 without seed re-run) still produces canonical state.

Source-of-truth modules:
- `src/lib/user-rules.ts` — frontend helper with `getUserRules(profile)` that reads columns with safe fallback to `RULE_DEFAULTS` (so partial profiles mid-onboarding don't break). Also exports `formatSafetyNet()` and `formatImpulseWait()` display helpers so the rendered strings come from a single function.
- `supabase/functions/_shared/user-rules.ts` — Deno-side mirror with the same `RULE_DEFAULTS` and a `getUserRulesFromProfile()` helper. Both files carry a "⚠️ Mirror file" comment explaining the duplication is structural (Edge Functions run in Deno and can't import from `src/`).

`prompt_builder.ts` rewired to call `getUserRulesFromProfile()` and render rule strings dynamically from profile values. Pre-0.5t had hardcoded English sentences with literal ₹ values in the grounding context block; post-0.5t the values flow from the profile row through the helper to the prompt. Side-finding 1 drift (₹2,000 prompt vs ₹3,000 UI) closed by construction — there's no separate hardcoded copy that can fall out of sync. `ProfilePage.tsx` "Your rules" section also rewired through the same helper, so the displayed Profile values + the AI's grounding values come from the same source row.

#### D.50 Onboarding "Your rules" step — user captures own thresholds during onboarding
Stream 0.5t piece #6. New step inserted in `OnboardingPage.tsx` between LifeAnchor (step 6) and FocusGoal (was step 7, now step 8). `TOTAL_STEPS` bumped from 7 to 8; Ready becomes step 9, Interstitial step 10. Step captures three inputs: safety net (₹ input), impulse-wait threshold (₹ input), impulse-wait duration (24/48/72 hour chips, defaulting to 48). Daily-SPS floor stays auto-derived per spec — not user input in V1; banked as V2 polish if any user actually cares to set it.

For the Priya demo path: input fields are pre-populated with the canonical values (₹1,00,000 / ₹3,000 / 48) so the user walking through onboarding sees them filled, just like income and anchor day. They can accept (Continue) or edit-then-accept; either way the state captures their choice. Per C.18 (onboarding is useState-only — zero DB writes for the demo path), the captured values don't propagate to the profile row; that's the V2 work for real new users. Validation gate: `canContinue` requires `safety_net ≥ ₹1,000`, `impulse_threshold ≥ ₹100`, `impulse_hours` in `[1, 168]`. Submit-blocked on invalid inputs.

UI follows the LifeAnchor pattern (StepFrame + StepTitle + ScrollBody + Footer; Card-wrapped inputs with ₹ prefix; hour-selector chips matching the anchor-day chip pattern; Sparkles-prefixed explanatory note at the bottom). Step title: "Your rules" with subtitle "Two thresholds Savio will reference when it weighs in on spending decisions. Defaults work; tweak if you've thought about it." Caps the friction-vs-meaning balance — most users tap Continue, a few will edit because the case-study target moment is "the AI honors rules the user actually set."

#### D.51 Chat grounding — drift fix + citation mandate extended to verdict_line + body
Stream 0.5t piece #7. Two changes bundled because they're in the same prompt:

(a) **Drift fix.** Rule values in the grounding context block are now interpolated from `getUserRulesFromProfile(profile)` — same source-of-truth as the Profile UI per D.49. The previous hardcoded English sentences with literal ₹2,000 / ₹3,000 / etc. are gone. Side-finding 1 closed by construction.

(b) **Citation extension.** The pre-0.5t prompt instructed the AI to reference rules in `tradeoffs[]` and `best_next_step` only — not `verdict_line` or `body`. New VERDICT_LINE RULES block mandates: when the spend amount is above `impulse_wait_threshold`, name the threshold value explicitly inside `verdict_line` (example given: *"Think twice — this ₹4,000 watch is over your impulse-wait threshold of ₹3,000."*). When the spend would push accessible cash below `safety_net`, name the safety net explicitly inside `verdict_line` (*"Step back — this would drop you below your safety net of ₹1,00,000."*). New BODY RULES block mandates that when `verdict_line` references a rule, `body` restates the rule briefly (*"Your rule says wait 48 hours over ₹3K"*) — adding context, not replacing the math.

Resists fabrication: explicit "If no rule is crossed, verdict_line stays focused on the math. Don't fabricate rule violations." Rule values that the AI emits are tied to the actual `impulse_wait_threshold` / `safety_net` values in the prompt — so for any non-Priya user with different values, the AI will cite the user's actual thresholds, not Priya's hardcoded ones.

Sample-opener block updated to use the dynamic threshold values instead of literal hardcoded ones. Pre-0.5t samples said "₹50K equals 4 months of safe-to-spend" — post-0.5t they reference the user's actual safety_net / impulse_wait_threshold values.

#### D.52 `rule_citations[]` on structured verdict response — programmatic rule-reference declaration
Stream 0.5t piece #8. Added `rule_citations: ("safety_net" | "impulse_wait" | "daily_sps_floor")[]` to the structured verdict contract in `prompt_builder.ts`. The AI declares which rules it actually referenced in the four content fields. Empty array if no rule applied — explicit empty rather than absent, so the field is always populated for new responses.

Prompt instruction: *"For every user rule you actually referenced (in verdict_line, body, tradeoffs[], or best_next_step), add its slug to rule_citations[]. Empty array if no rule applied. Don't fabricate rule violations to populate the array."* The array drives two downstream things:

(a) **Future programmatic verification.** Similar to D.18's hallucination guard for rupee values, a `rule_citation_guard` could validate that if `verdict_line` mentions a threshold value, the matching slug appears in `rule_citations`. Not built in 0.5t — banked as polish. The array's existence enables the work.

(b) **VerdictCard badge rendering.** Frontend `VerdictCard.tsx` reads `rule_citations` and renders a small chip above the tradeoffs callout for each cited rule: Shield icon for `safety_net`, Clock for `impulse_wait`, Wallet for `daily_sps_floor`. Chip style: 10.5px text, muted color, sage-tinted background (`rgba(59,109,17,0.06)`) with hairline border, 999px pill. Tooltip: *"Verdict references your {rule name}."* Hidden when array is empty or absent (old saved_decisions rows from pre-0.5t render cleanly with no badges).

`src/lib/chat-types.ts` updated: `StructuredVerdict.rule_citations` typed as optional `RuleCitationSlug[]` (optional for backward compatibility with cached/saved verdicts that pre-date 0.5t); `isValidStructured()` validator passes both presence and absence, rejects invalid slugs if present. Single-source `RuleCitationSlug` enum mirrors the slugs the Edge Function emits — no string magic on either side.

#### D.53 Chat audit re-baseline + impulse-wait boundary Cases 8 + 9
Stream 0.5t piece #9. Updates to `scripts/test-chat-7cases.mjs`:

- **Case 1** expectation updated from "₹12,032" to "₹41,532" per D.47 income raise
- **Case 2** expectation updated to verify rule citation: *"₹41,532 → ₹36,532 remaining; verdict_line cites impulse-wait threshold (₹3K) per D.51; rule_citations includes 'impulse_wait'; Verified; Save Decision visible"*
- **Case 3** kept (Myntra regret rate at 87.5% per D.38)
- **Cases 4-7** unchanged (don't touch income or rules)
- **NEW Case 8** — "Can I afford a ₹2,500 watch?" — tests the AI does NOT cite the impulse-wait rule when amount is below threshold (₹2,500 < ₹3,000). Expected: `rule_citations: []`, no threshold mention in verdict_line. Discipline check that the model doesn't fabricate rule violations to look thorough.
- **NEW Case 9** — "Can I afford a ₹3,500 watch?" — tests the boundary just above threshold. Expected: `rule_citations` includes `"impulse_wait"`, verdict_line cites threshold, body restates the 48-hour rule. The case-study target moment in a single audit row.
...
#### D.55 Windfall card dismiss — remove X, wire Skip via session state
Stream 0.5u piece #3 (Spec banked these as D.54-D.58 but D.54 was already in use for the SafeToSpendHero relabel committed earlier the same day; banked as D.55-D.59 with the collision noted). Real-user testing found both dismiss affordances on the Home windfall card (Skip + X cross) silently non-functional. Root cause was double: (a) `WindfallCard.tsx` had both buttons calling the same `onDismiss` handler — no UX value in two affordances calling one handler; (b) `HomePage.tsx` was rendering `<WindfallCard amount=... onAllocate=... />` with NO `onDismiss` prop passed — so both buttons were bound to `undefined` and silently did nothing on tap.

Fix:
- Removed the X-cross button entirely from `WindfallCard.tsx`. Single Skip affordance remains.
- Added `windfallSkipped` session-state flag in `HomePage.tsx`. Card renders only when `pendingWindfall && !windfallSkipped`. Skip taps fire `() => setWindfallSkipped(true)`.
- Session-only: the windfall itself stays `pending_allocation` in DB; only the card visibility is suppressed. Component unmount keeps the flag while React state is alive (navigating Home→Chat→Home stays skipped). Full page reload or auto-reset (D.15) clears it.

Single-affordance model fixes the bug by reducing surface area rather than debugging two broken handlers. Pairs with C.28 / 0.5p Piece #9 (windfall labeling) — the card now has correct identity + correct dismiss in one place.

#### D.56 Windfall destination page renamed "Windfall" → "Allocate"
Stream 0.5u piece #2. Real-user testing found the handoff incoherent: Home windfall card's primary CTA reads "Allocate now"; the destination page header read "Windfall". User tapped one word, landed on another. Renamed `<RitualHeader sectionLabel="Windfall">` → `sectionLabel="Allocate"` on both flow steps (`WindfallAllocate.tsx` step 1, `WindfallReview.tsx` step 2). Page identity is now the action; the windfall identity persists via the C.28 labeled header inside the page body (`"Diwali bonus · ₹50,000"`).

"Windfall" remains the right word for the underlying event (a Diwali bonus IS a windfall); the page is about the action of allocating, so its title is the action. Same family as D.55 and the voice-discipline thread (C.16 / C.26 / D.27 / D.34 / D.48): make labels self-explanatory.

#### D.57 "Commitments" → "Fixed commitments" across user-facing surfaces
Stream 0.5u piece #1. Real-user testing found bare "Commitments" insufficiently self-explanatory — users didn't immediately grasp it meant their fixed recurring obligations as distinct from spending categories or goals. Renamed where the `kind='fixed'` commitments appear:

(a) **Home CommitmentsCard**: title "Commitments on track" → "Fixed commitments on track" (`src/components/home/CommitmentsCard.tsx`)

(b) **Profile section**: header "Your commitments" → "Your fixed commitments" (`src/pages/ProfilePage.tsx`). The expand/contract pattern from D.37 unchanged.

(c) **Chat grounding context**: prompt section heading "Non-investing commitments (outflow — ...)" → "Fixed commitments (outflow, non-investing — ...)" (`supabase/functions/chat-respond/prompt_builder.ts`). Subtitle "(outflow, non-investing)" preserves the math-relevant subset distinction for the AI without re-introducing finance-jargon to the user. The AI naturally adopts whatever heading text is in the grounding context, so renaming the heading propagates to the AI's own language when discussing them.

Variable spending categories remain "Spending categories" (D.20 unchanged). Investing commitments (SIPs) remain a separately-labeled section in the chat prompt — they're `kind='fixed'` schema-wise but functionally different (savings flow, not outflow). In Profile the "Your fixed commitments" section includes all 13 `kind='fixed'` rows (10 non-investing + 2 SIPs + Maid); the SIPs/Investing distinction matters only to the safe-to-spend math, not to the user's "what have I committed to" mental model.

Pairs with C.16 / C.26 / D.27 / D.34 / D.48 — same voice-discipline family that runs through the build.

#### D.58 Removed "Patterns this week" section from Home
Stream 0.5u piece #4. Real-user testing found the `PatternsCallout` section ("Patterns this week") above the Home transactions list non-functional and adding cognitive load without value. Removed the JSX render + import from `HomePage.tsx` lines 17 + 269. Component file `src/components/home/PatternsCallout.tsx` left dormant (same discipline as `synthesize-patterns/index.ts` per D.40 — code preserved for git history; not imported, not rendered).

`generateGuidance()` in `lib/guidance.ts` still computes a `reflectionPattern` object alongside `focusGoal`; the unused field stays in the return value (low overhead, removing it would require refactoring `generateGuidance` callers). If a future surface wants reflection-pattern insights on Home, the data path is intact.

Distinct from the Reflect tab trend cards + emotion chart (0.5p–0.5s) which are unaffected. The pattern-analysis story lives on the Reflect tab where it belongs; Home now focuses on safe-to-spend, fixed commitments, goals tracking, windfall allocation, and recent transactions. Reduces Home cognitive load.

#### D.59 "For you today" → "Track your goals" on Home
Stream 0.5u piece #5. Real-user testing found "For you today" vague. Pre-flight confirmed the section's content (via `lib/guidance.ts::FocusGoalInsight`) is always goal-progress insights (focus-goal status, target-amount tracking, target-date countdown). Renamed `<SectionHeader title="For you today">` → `<SectionHeader title="Track your goals">` in `src/components/home/ForYouTodayCard.tsx`. Concrete and action-oriented; matches what's actually rendered. Pairs with the voice-discipline family.

#### D.60 Close-out math-reveal recap card — delivers the never-shipped D.26
Stream 0.5v pieces #1, #2, #3 combined. The original D.26 (Stream 0.5p Piece #10) was specced — "Spending leftover" rename + math-reveal recap card — but never made it into a Claude Code stream. PM_DECISIONS jumped from D.25 to D.27 with no D.26 entry, and the close-out screen kept rendering "Discretionary leftover" with the old subtitle. Diagnostic on the live deployed close-out surfaced that the math was correct but the presentation was lying by omission: a huge red deficit number (`total_leftover`) sitting next to a similar-shaped red "Discretionary leftover" number, with three visible green spending categories totaling +₹5K net and no breakdown of the ~₹30K of one-off discretionary spend driving the actual deficit.

Three changes shipped together:

(a) **Math-reveal recap card** (`MonthlyRitualCloseOut.tsx`). New Card with header "The math" rendering one row per component: Income (+), Fixed commitments (−), Goal contributions (−), Variable category net (±), One-off spending (−, tappable to expand), then a hairline divider and the bolded "Spending leftover" total row. Every value flows from a new `recap` object the Edge Function returns; the UI doesn't re-derive. Positive rows render `#173404` (dark green), negatives `#501313` (dark red), tabular-nums for clean column alignment.

(b) **One-off spending tap-to-expand**. The Edge Function returns `one_off_breakdown = { top: MerchantTotal[4], other_total, other_count, total, full_list }` where `top` is the four largest null-commitment merchants in the month and the remainder bucket into "N others". The recap row uses a chevron-down that rotates 180° on expand; expanded view indents the merchant rows in muted gray. Resolves diagnostic Scenario A (correct-but-invisible) — the user can audit row by row where the deficit came from.

(c) **"Discretionary leftover" → "Spending leftover" rename** (the D.26 rename, finally landing). The standalone Card that hosted the old label is removed; the rename surfaces on the recap's total row instead. Subtitle clarifies: *"Income minus commitments minus your actual spending. Negative means you went past your safe-to-spend."* Resolves diagnostic Scenario C (component + total were shown as sibling Cards, confusing the user into reading them as two competing claims).

Edge Function changes in `supabase/functions/ritual-close-out/index.ts`: added `one_off_breakdown` builder that groups null-commitment debits by merchant; added `recap` typed sub-object surfacing the component values; both included in the JSON response alongside the pre-existing `total_leftover` / `discretionary_leftover` (kept on the wire for backward compat with any other caller).

#### D.61 Close-out "Looking back" reflection-labeling section removed
Stream 0.5v piece #4. The close-out screen had a "Looking back" section asking the user to label top discretionary transactions as Worth-it / Neutral / Regret — `unlabeled_transactions` payload field consumed by `ReflectionLabelRow`. After Streams 0.5p–0.5s built the dedicated Reflect tab (per-merchant trend cards, emotion line chart, AI prose now rule-engine prose), reflection-labeling has a single canonical home; asking again at close-out was duplicative + diluted the close-out's purpose (which is to close out the past month, not to label transactions). Removed the section + the related `handleReflectionLabel` handler + the `ReflectionLabelRow` / `ReflectionLabel` imports. Edge Function still returns `unlabeled_transactions` on the wire for backward compat — just not rendered. Reflect tab unaffected.

#### D.62 "What you can do now" close-out guidance — rule-engine, four severity tiers
Stream 0.5v piece #5. The load-bearing piece of the stream. When a user finishes a month in the yellow/red zone (net leftover negative OR small-positive under ₹5K), the close-out screen renders a "What you can do now" card. Locked principle: *the truth about a deficit shouldn't sit alone inducing anxiety — pair it with a constructive path.* Rule-engine generated (consistent with D.40 hallucination-avoidance — no LLM), deterministic, names the real driver merchants + offers a concrete lever tied to the user's own stated rules (impulse-wait threshold/hours, safety net amount). Four severity tiers checked in priority order:

| Tier | Trigger | Tone |
|---|---|---|
| `repeated_deficit` | 2+ consecutive deficit months | Honest pattern flag: "not one to ride out" |
| `deficit_breached` | Accessible cash < safety net | More serious: rebuild safety net is the priority |
| `deficit_safe` | Deficit but safety net intact | Course-correct + impulse-wait lever; "not an alarm" |
| `small_short` | 0 to ₹5K positive | Gentle nudge to Reflect |

Implementation in `supabase/functions/ritual-close-out/index.ts::deriveCloseOutGuidance()`. UI in `MonthlyRitualCloseOut.tsx::GuidanceCard`. Tier-conditional card tint (calm light-blue `#F0F4F8` for the lighter tiers, light amber `#FFF4E8` for the more serious ones — body copy carries the seriousness, not the chrome).

**Calibration discipline**:
- The body copy for `deficit_safe` names the top-3 driver merchants, the impulse-wait rule by value (e.g. *"48 hours over ₹3,000"*), and computes *"would have given you a pause on ₹X"* by summing one-off purchases above the threshold. Lever tied to the user's actual rules, not generic advice.
- No emoji, no exclamation cheerleading, no doom. The phrase *"course-correct, not an alarm"* is the locked framing for `deficit_safe`.
- `deficit_breached` flips the priority — explicitly names the safety net value being violated as the line to rebuild before the next discretionary spend.

**0.5v-shipped simplifications, documented**:
- `current_savings` proxied as the Emergency-fund goal's `current_amount`. The safety net is conceptually "minimum accessible cash to preserve"; for this single-user demo, that IS what the emergency fund represents. V2: real bank-balance integration.
- `consecutive_deficit_months` stubbed at 0 in the canonical Edge Function path. The `repeated_deficit` tier ships as designed-but-not-demoed; the demo flow exercises `deficit_safe` via the March seed (below). V2: track close-out outcomes on `monthly_rituals` and walk history backwards.

**March 2026 negative-month seed.** Added four high-impact null-commitment March transactions in `0006_seed_priya.sql` (IndiGo Airlines ₹18,000 Goa weekend / Royal China ₹6,500 dinner / Croma ₹4,500 earbuds / Forever 21 ₹9,200 apparel — total ~₹38K). With post-D.47 ₹98,000 income, March close-out math now lands ≈ −₹10K (safety net at ₹1,84,000 intact, so the `deficit_safe` tier fires). Reviewer Console gained a "Preview March close-out (deficit demo)" row that navigates to `/ritual/2026-03` — the route accepts any month string, so the existing close-out screen renders fine for past months.

**The strongest case-study line in the build:** *"The best time for the app to act is when the user needs to figure out their finances. A close-out that says '−₹15,146, good luck' is a dead end. A close-out that says '−₹15,146 — here's what's drivable next month, tied to your own rules' is decision-support doing its job at the moment of felt consequence."*
